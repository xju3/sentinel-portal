...
* **FFT 拉取任务**：`action=99, val=0`。任务不携带点数或量程，设备根据转速、采样圈数和削顶状态自行决定。
...
3. **最终确诊 (第 3 次, resampling=False)**：
   ...
   * 此记录代表正式确诊，系统放行对外推送，并下发 `action=99` 进行 FFT 采集。
...
* 在此 24 小时内，后续周期采集的异常数据**照常落库**（保证诊断历史连续性），但**强行拦截**新的复采任务、FFT 任务 (`99`) 以及对外推送。
...
**云端下发策略**：云端只下发 `action=99, val=0`。FFT 点数和量程由设备自行决定，实际值随 FFT 二进制文件上传，云端不从任务编码推导。
...
2. 数据经由 `sensors.py` 存入 MinIO `fft` bucket；API 随即向 Redis Stream 写入 `task_id`。
3. Diagnosis 的 FFT worker 消费消息，从文件读取元数据，生成 `DeviceFftRecord` 并执行独立 FFT 诊断。

### 4.2 二进制文件解析 (Binary Structure)
FFT 解析单元根据 `task_id` 从 MinIO 拉取文件，按严格的小端序 (Little-Endian) 结构解包：
* **Header (前 32 Bytes)**：
  * `0~15`: `SN` 字符串（16字节，null截断）。
  * `16~19`: `Timestamp` (uint32 Unix 秒级时间戳，服务端换算为毫秒)。
