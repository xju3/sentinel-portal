...
import uuid
from sqlalchemy import Column, String, Uuid, Integer, SmallInteger, Float, Boolean, Text
...
class DeviceComboSpec(Base):
    """Device combo specification entity model"""

    __tablename__ = "device_combo_spec"
...
class DeviceComboSpecItem(Base):
    """Device combo detail entity model"""

    __tablename__ = "device_combo_spec_item"

    id = Column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    device_combo_id = Column(Uuid(as_uuid=True), nullable=False, index=True)  # Link to device_combos
    device_spec_id = Column(Uuid(as_uuid=True), nullable=False, index=True)
    qty = Column(Integer, nullable=False, default=1)

    
class DeviceComboInst(Base):
...
class DeviceComboInstItem(Base):
